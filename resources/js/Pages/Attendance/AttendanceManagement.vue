    ...
    <div class="px-6 md:px-10 pt-10 pb-10">
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div class="flex gap-2">
                <div>
                     <input type="text" placeholder="Cari Data..." v-model="search"
        ...
        <div class="bg-white p-6 rounded-xl shadow border border-gray-100 mt-4 overflow-hidden flex flex-col h-[calc(100vh-260px)]">
             <div class="overflow-auto flex-1">
                <table class="w-full text-sm text-left text-gray-600 relative">
                    <thead class="text-xs text-gray-700 uppercase bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr v-else v-for="(item, index) in filteredAttendances" :key="item.id" class="hover:bg-blue-50 transition duration-150">
                            <td class="px-6 py-4 font-medium text-gray-900 text-center">{{ index + 1 }}</td>
                            <td class="px-6 py-4 font-medium text-gray-700">{{ item.transaction?.customers?.name || 'N/A' }}</td>
                            <td class="px-6 py-4 font-medium text-gray-700">{{ item.transaction?.customers?.email || 'N/A' }}</td>
                            <td class="px-6 py-4 font-medium text-gray-700 text-center">{{ item.transaction?.seats?.seat_number || 'N/A' }}</td>
                            <td class="px-6 py-4 font-medium text-gray-700 text-center">{{ formatDateTime(item.attendance_time) }}</td>
                            <td class="px-6 py-4 font-medium text-gray-700 text-center">{{ formatDateTime(item.leave_time) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 relative">
                ...
                <div v-if="scanMessage" :class="{'bg-green-50 border-green-200 text-green-700': scanStatus === 'success', 'bg-red-50 border-red-200 text-red-700': scanStatus === 'error'}" class="p-4 border rounded-lg text-center mt-4 mb-4">
                    <p class="text-sm font-bold">{{ scanMessage }}</p>
                </div>

                <!-- Recent Scans Section -->
                <div v-if="recentScans.length > 0" class="mt-2 mb-6 w-full border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <ul class="max-h-30 overflow-y-auto divide-y divide-gray-100">
                        <li v-for="(scan, idx) in recentScans" :key="idx" class="px-4 py-3 bg-white hover:bg-gray-50 flex justify-between items-center transition">
                            <div class="flex flex-col">
                                <span class="font-semibold text-gray-800 text-sm">{{ scan.transaction?.customers?.name || 'Unknown' }}</span>
                                <span class="text-xs text-gray-500">{{ scan.transaction?.seats?.seat_number || '-' }} | {{ scan.scan_type === 'checkout' ? 'Check Out' : 'Check In' }}</span>
                            </div>
                ...
                <div v-if="cameras.length > 0" class="mt-6 flex flex-col items-center gap-4">
                    ...
                    <div class="flex gap-4 w-full justify-center">
                        <button v-if="isScanning" @click="stopScanner" class="px-6 py-2 bg-rose-500 hover:bg-rose-600 text-white font-semibold rounded-lg shadow-md transition w-full sm:w-auto">
                            Hentikan Scanner
                        </button>
                        <button v-else @click="startScanner" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-md transition w-full sm:w-auto">
                            Mulai Scanner
                        </button>
                    </div>
                </div>
...
const filteredAttendances = computed(() => {
    ...
    return attendances.value.filter(item => {
        const nameMatch = item.transaction?.customers?.name?.toLowerCase().includes(lowerSearch);
        const emailMatch = item.transaction?.customers?.email?.toLowerCase().includes(lowerSearch);
        const seatMatch = item.transaction?.seats?.seat_number?.toLowerCase().includes(lowerSearch);
        
        return nameMatch || emailMatch || seatMatch;
    });
});
...
const fetchAttendances = async () => {
    try {
        ...
        attendances.value = response.data.data.sort((a,b) => new Date(b.created_at) - new Date(a.created_at));
    } catch (error) {
