            ...
            <main class="p-6">
                <form @submit.prevent="submitForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="name" type="text" placeholder="Masukkan Nama Makanan"
...
const judul = computed(() => {
    return props.isEdit ? 'Ubah Makanan' : 'Tambah Makanan';
});
...
const emit = defineEmits(['close', 'fetchFoods']);
...
async function submitForm() {
    try {
        ...
        if (response.status === 200) {
            ...
            alert('Makanan berhasil ' + (props.isEdit ? 'diupdate' : 'ditambahkan'));
            emit('close');
            emit('fetchFoods');
        }
    } catch (error) {
